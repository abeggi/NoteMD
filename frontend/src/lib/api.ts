...
export class ApiClient {
  ...
  constructor() {
    ...
    if (typeof window !== "undefined") {
      this.token = localStorage.getItem("notemd_token");
    }
  }
  ...
  setToken(token: string) {
    this.token = token;
    if (typeof window !== "undefined") {
      localStorage.setItem("notemd_token", token);
    }
  }
  ...
  async isOrgAdmin(): Promise<boolean> {
    ...
    try {
      ...
      localStorage.setItem("notemd_current_org", JSON.stringify(updatedOrg));
  ...
  async exportDocuments() {
    ...
    a.href = url;
    a.download = `notemd-export-${new Date().toISOString().slice(0, 10)}.tar.gz`;
