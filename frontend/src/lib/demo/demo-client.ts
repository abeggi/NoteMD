...
export const demoClient = {
  ...
  async login(username: string, password: string) {
    ...
    // Set fake token
    localStorage.setItem("notemd_token", "demo-token");
    localStorage.setItem("notemd_current_org", String(org.id));

    return { user, token: "demo-token" };
  },

  async logout() {
    localStorage.removeItem("notemd_token");
    localStorage.removeItem("notemd_current_org");
  },

  async validateSession() {
    await ensureReady();
    const token = localStorage.getItem("notemd_token");
  ...
  async getCurrentOrganization() {
    ...
    const orgStr = localStorage.getItem("notemd_current_org");
    if (orgStr) {
      try {
  ...
  async exportDocuments() {
    ...
    a.href = url;
    a.download = `notemd-demo-export-${new Date().toISOString().slice(0, 10)}.json`;
      ...
      reader.onload = (e) => {
        try {
          ...
          const stored: any[] = JSON.parse(
            localStorage.getItem("notemd_demo_attachments") || "[]",
          );
  ...
  async deleteAttachment(attachmentPath: string) {
    ...
    localStorage.setItem("notemd_demo_attachments", JSON.stringify(filtered));
